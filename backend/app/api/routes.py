...
def upload_transactions(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    for _, row in df.iterrows():
        ...
        tx = Transaction(
            ...
            amount=float(row["amount"]) if "amount" in row and not pd.isna(row["amount"]) else None,  # type: ignore
            category=str(row["category"]),
            unit=str(row["unit"]),
            quantity=float(row["quantity"]),
            emission_scope=str(row["emission_scope"]) if "emission_scope" in row and not pd.isna(row["emission_scope"]) else None,  # type: ignore
        )
...
def list_transactions(db: Session = Depends(get_db)):
    """
    Return all stored transactions with their computed emissions.
    """
    records = db.query(Transaction).order_by(Transaction.date.desc()).all()
    return records
...
def emissions_summary(db: Session = Depends(get_db)):
    ...
    Example JSON:
    {
      "scope1": {"natural_gas": 690.0},
      "scope2": {"electricity": 1650.0},
      "scope3": {"flight": 1800.0}
    }
    """
    rows = (
